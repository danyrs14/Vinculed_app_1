...
// Helpers locales (idénticos a versión web)
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [
          BoxShadow(blurRadius: 10, offset: Offset(0, 2), color: Color(0x0F000000))
        ],
      ),
      child: Column(
          ...
          children: [
            Text(title,
                style: const TextStyle(
                    fontWeight: FontWeight.w700, color: Color(0xFF22313F))),
...
class _LocalDateTimePickerField extends StatelessWidget {
  const _LocalDateTimePickerField(
      {required this.label,
  ...
  Widget build(BuildContext context) {
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        floatingLabelStyle:
            TextStyle(color: theme.fuente(), fontFamily: 'Poppins'),
        border: const OutlineInputBorder(),
        enabledBorder: OutlineInputBorder(
            borderSide: BorderSide(color: theme.secundario())),
        focusedBorder: OutlineInputBorder(
            borderSide: BorderSide(color: theme.secundario(), width: 1.4)),
        isDense: true,
...
class _LocalHabilidadesBlandas extends StatefulWidget {
  ...
  @override
  State<_LocalHabilidadesBlandas> createState()=> _LocalHabilidadesBlandasState();
}
...
class _LocalHabilidadesBlandasState extends State<_LocalHabilidadesBlandas> {
  ...
  @override
  Widget build(BuildContext context) {
    return _LocalCardBox(
        title: 'HABILIDADES BLANDAS:',
        child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              HabilidadesMultiDropdown(
                  label: 'Selecciona habilidades blandas e idiomas',
                  hintText: '',
                  allowedTipos: const ['blanda', 'idioma'],
                  initialSelectedIds: _ids,
                  onChanged: (list) {
                    setState(() => _ids = list.map((e) => e.id).toList());
...
class EditarVacantePage extends StatefulWidget {
  const EditarVacantePage({super.key, this.idVacante});
  final int? idVacante;
  @override
...
class _EditarVacantePageState extends State<EditarVacantePage> {
  ...
  Future<void> _fetchVacante() async {
    final id = widget.idVacante; if(id==null) return; setState(()=> _loadingData = true); _loadError = null;
    try {
      ...
      _fillForm(data); setState(()=> _loadingData=false);
    } catch(e){ setState((){ _loadError='Excepción: $e'; _loadingData=false; }); }
  }

  // Utils fecha
  String _fmt(DateTime dt){ String two(int v)=> v.toString().padLeft(2,'0'); return '${dt.year}-${two(dt.month)}-${two(dt.day)} ${two(dt.hour)}:${two(dt.minute)}:00'; }
  String _fmtIsoToInput(String? iso){ if(iso==null || iso.isEmpty) return ''; try{ final dt = DateTime.parse(iso).toLocal(); return _fmt(dt); } catch(_){ return iso; } }
  DateTime? _tryParseInput(String? s){
    ...
    if(onlyDate.hasMatch(t)){
      try{ final parts = t.split('-'); return DateTime(int.parse(parts[0]), int.parse(parts[1]), int.parse(parts[2]), 0,0,0); } catch(_){ /* ignore */ }
    }
  ...
  void _fillForm(Map<String, dynamic> d) {
    ...
    _fechaInicioCtrl.text = _fmtIsoToInput(d['fecha_inicio']?.toString());
    _fechaFinCtrl.text = _fmtIsoToInput(d['fecha_fin']?.toString());
    ...
    _fechaLimiteCtrl.text = _fmtIsoToInput(d['fecha_limite']?.toString());
    ...
    final roles = (d['roles_relacionados'] as List? ?? []).map((e) {
      if (e is Map<String, dynamic>) {
        return RoleOption(
            id: e['id_roltrabajo'] ?? e['id'] ?? 0,
            area: e['area']?.toString() ?? '',
            nombre: e['nombre']?.toString() ?? (e['rol']?.toString() ?? 'Rol'));
      }
    ...
    _habTecnicasIds = []; _habBlandasIds = [];
    for (var h in habs) {
      if (h is Map<String, dynamic>) {
        final tipo = (h['tipo']?.toString() ?? '').toLowerCase();
        final idH = h['id_habilidad'] ?? h['id'] ?? 0;
        if (tipo.contains('técn')) _habTecnicasIds.add(idH); else _habBlandasIds.add(idH);
      }
    }
  }
  ...
  Future<void> _guardarCambios() async {
    ...
    // Nueva validación: inicio no antes de hoy
    if (oInicio != null && oInicio.isBefore(hoy)) {
      errores.add('La fecha de inicio no puede ser menor que hoy.');
    }
    // Nueva validación: fin no antes de hoy
    if (oFin != null && oFin.isBefore(hoy)) {
      errores.add('La fecha de fin no puede ser menor que hoy.');
    }
    if (oLimite != null && oInicio != null && oLimite.isAfter(oInicio)) {
      errores.add('La fecha límite debe ser anterior o igual a la fecha de inicio.');
    }
    if (oLimite != null && oLimite.isBefore(hoy)) {
      errores.add('La fecha límite no puede ser menor que hoy.');
    }
    if (oFin != null && oInicio != null && oFin.isBefore(oInicio)) {
      errores.add('La fecha de fin no puede ser menor que la fecha de inicio.');
    }
    ...
    try {
      ...
      final resp = await http.put(
          Uri.parse('https://oda-talent-back-81413836179.us-central1.run.app/api/reclutadores/editar_vacante'),
          headers: headers,
          body: jsonEncode(payload));
      if (!mounted) return;
      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cambios guardados')));
        Navigator.pop(context, true); // devolver bandera para refrescar detalle
      } else {
                        ...
                        child: Form(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              RolesMultiDropdown(
                                  label: 'Roles de trabajo relacionados',
                                  hintText: '',
                                  initialSelectedIds:
                                      _rolesSeleccionados.map((e) => e.id).toList(),
                                  onChanged: (r) =>
                                      setState(() => _rolesSeleccionados = r)),
                              const SizedBox(height: 12),
                              StyledTextFormField(
                                  controller: _salarioCtrl,
                                  title: 'Monto de beca mensual',
                              ...
                              _LocalCardBox(
                                ...
                                child:
                                    HabilidadesMultiDropdown(
                                        ...
                                        hintText: '',
                                        allowedTipo: 'tecnica',
                                        initialSelectedIds: _habTecnicasIds,
                              ...
                              isMobile
                                  ? Column(children:[
                                      _LocalDateTimePickerField(
                                          label: 'Fecha inicio',
                                          controller: _fechaInicioCtrl,
                                          onTap: () =>
                                              _pickDateTime(controller: _fechaInicioCtrl)),
                                      const SizedBox(height: 12),
                                      _LocalDateTimePickerField(
                                          label: 'Fecha fin',
                                          controller: _fechaFinCtrl,
                                          onTap: () =>
                                              _pickDateTime(controller: _fechaFinCtrl)),
                                      const SizedBox(height: 12),
                                      StyledTextFormField(
                                  ...
                                  : Row(children:[
                                      Expanded(
                                          child: _LocalDateTimePickerField(
                                              label: 'Fecha inicio',
                                              controller: _fechaInicioCtrl,
                                              onTap: () =>
                                                  _pickDateTime(controller: _fechaInicioCtrl))),
                              ...
                              isMobile
                                  ? Column(children:[
                                      ...
                                      _LocalDateTimePickerField(
                                          ...
                                          onTap: () =>
                                              _pickDateTime(controller: _fechaLimiteCtrl)),
                                    ])
                                  : Row(children:[
                                      Expanded(
                              ...
                              _LocalHabilidadesBlandas(
                                  initialSelectedIds: _habBlandasIds,
                                  onSelected: (list) {
                                    _habBlandasIds = list.map((e) => e.id).toList();
                                  }),
                              const SizedBox(height: 12),
                              StyledTextFormField(
