...
class PublicacionCard extends StatelessWidget {
  ...
  /// Texto de estado (por defecto "PUBLICADA")
  ...
  /// Constructor de conveniencia para estado "PUBLICADA".
  const factory PublicacionCard.publicado({
    Key? key,
  ...
  Future<void> _handleUnapply(BuildContext context, ThemeController theme) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Eliminar Vacante', style: TextStyle(color: theme.fuente())),
        content: Text('¿Quieres quitar esta vacante?', style: TextStyle(color: theme.fuente())),
        actions: [
          MiniButton(
  ...
  Widget build(BuildContext context) {
    ...
    final content = Container(
      ...
      decoration: BoxDecoration(
        ...
        border: showBorder ? Border.all(color: theme.secundario(), width: 1) : null,
    ...
    return Material(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(borderRadius),
        // Si te pasan un onTap, se usa; si no, navegamos al detalle por ruta nombrada.
        ...
        onLongPress:
        onUnapply == null ? null : () => _handleUnapply(context, theme),
        child: content,
      ),
    );
  }
...
/// Implementación concreta del constructor de conveniencia `publicado`.
class _PublicacionCardPostulado extends PublicacionCard {
  const _PublicacionCardPostulado({
    super.key,
