...
class RemoteReply { // Comentario directo a la publicación
...
class RemoteCommentReply { // Respuesta a un comentario
  final int idComentario;
  final int idAlumno;
  ...
  factory RemoteCommentReply.fromJson(Map<String,dynamic> j) => RemoteCommentReply(
    idComentario: (j['id_comentario'] as num?)?.toInt() ?? 0,
    idAlumno: (j['id_alumno'] as num?)?.toInt() ?? 0,
...
class InlineReplyComposer extends StatelessWidget {
  ...
  final VoidCallback onCancel;
  final VoidCallback onSend;
  const InlineReplyComposer({required this.controller, required this.sending, required this.onCancel, required this.onSend});
  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
